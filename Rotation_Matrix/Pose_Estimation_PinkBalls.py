...
while(True):
        ...
        if len(sliced) != 7:
            ...
            try:
                ...
                XYZ = 410*np.dot(np.linalg.inv(camera_matrix), xy1.T)
                print "XYZ",XYZ
                # print camera_matrix
                ...
                rodrigues = cv2.Rodrigues(rotation_vector)[0]
                euler_converted = yawpitchrolldecomposition(rodrigues)
                UVW=np.dot(rodrigues.T, translation_vector)
                print "UVW",UVW

                yawpitchroll_angles = -180*yawpitchrolldecomposition(rodrigues)/math.pi
                ...
                x_theta = np.deg2rad(yawpitchroll_angles[2])  # add gimbals reported camera angle to x
                z_theta = np.deg2rad(yawpitchroll_angles[0]) # rotation_vector[1]
                y_theta = np.deg2rad(yawpitchroll_angles[1])  # rotation_vector[2]

                a = translation_vector[2]
                b = translation_vector[0]
                ...
                print "euler_converted",euler_converted
                d = math.tan(np.deg2rad(yawpitchroll_angles[1]))*translation_vector[2]
                alpha = 90-yawpitchroll_angles[1]
                beta = 90-alpha
                x = math.tan(np.deg2rad(beta))*d
                TOTAL = translation_vector[1]+math.tan(np.deg2rad(yawpitchroll_angles[1]))*(d+x)

                drone_pos = np.dot(-rodrigues.T, translation_vector)
                print "drone_pos", drone_pos

                print "d",d
                print "true drone y dist",d+translation_vector[1]
                ...
                print "rotation_vector", format((yawpitchroll_angles))  # radians (x,y,z)
                N = rotation_vector[1]
                D = rotation_vector[2]
                E = (rotation_vector[0]-1.5708)*-1
                NED = [N,E,D]
                print "NED,xyz,rpy", format(np.rad2deg(NED))
